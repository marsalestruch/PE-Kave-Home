...
def biggest_dimension(x):
    """
    :param x: (x1,x2,x3). mides de caixa o objecte
    :return: major dimensió de x
    """
    sol = 0
    for xi in x:
        sol = max(sol, xi)
...
def fits(obj, box):
    ...
    """
    assert len(obj) == 3 and len(box) == 3
    assert obj[0] <= obj[1] <= obj[2]
    assert box[0] <= box[1] <= box[2]
    return all([oi <= bi for oi,bi in zip(obj,box)])
...
def total_space_obj(objects, obj_quants):
    ...
    """
    assert len(objects) == len(obj_quants)
...
def total_space_box(box_sizes, obj2box, obj_quants):
    ...
    assert obj2box.min() >= 0 and obj2box.max() <= len(box_sizes)-1
...
def compute_impact(objects, obj_quants, obj2box, box_sizes):
    ...
    # TODO revisar tot això, arribar a un acord. Dona massa alt.

    preu_bobina = 207.92  #euros
    len_bobina = 128000  #centimetres (1280 metres)

    L = 30  #longitud del cilindre en cm
    r = 6.5  #radi del cilindre en cm
    volum_bossa = L * r*r * 3.1416  #volum d'un cilindre = π·r²·h [cm³]
    volum_bossa *= 0.7   #suposem que la bossa ocupa un X% del cilindre
    ...
    for obj_id in range(len(objects)):
        ...
        empty_space = int(empty_space * 0.3)

        # Ponderem pel nombre d'objectes per saber el nombre total de bosses
        # necessari per a tots els objectes d'aquest obj_id
        num_bosses += obj_quant * (empty_space // volum_bossa)

    # (€/cm de bobina) * (cm gastats per bosa)
    preu_bossa = (preu_bobina / len_bobina) * (2*r)
...
def compute_obj2box(objects, box_sizes):
    ...
    b = sum([not fits(objects[i], box_sizes[obj2box[i]]) for i in range(len(objects))])
    if b:
        print(b)
        raise Exception("SSSSSSSSSSSSSSSSSSSSSSSS")
...
def run_validations(limit_obj_idxs=None, box_sizes=None,
                    obj2box=None, objects=None):
    ...
    if obj2box is not None:
        ...
        for obj, box_id in zip(objects, obj2box):
            ...
            if not fits(obj, box_sizes[box_id]):
                print(obj, box_sizes[box_id])
                raise Exception
...
def create_mutation(objects, obj_quants, w=5):
    ...
    new_quants = obj_quants.copy()

    for i in range(0, len(objects), w):
        _w = min(w, len(objects)-i-1)
...
def ComputeSizes(_objects, _obj_quants, num_boxes, num_iter=300,
    ...
    for ii in range(num_iter):
        ...
        if p < prob_mutation or iters_without_improvement > iters_to_mutation:
            ...
            old_space = total_space_obj(_objects, _obj_quants)
...
def ComputeBaselineSizes(_objects, _obj_quants, num_boxes, method="volume"):
    ...
    if method == "volume":
        idx_sorted = sorted(range(len(objects)), key=lambda k: np.prod(objects[k]))

    elif method == "diagonal":
        idx_sorted = sorted(range(len(objects)), key=lambda k: diagonal(objects[k]))

    elif method == "biggest_dimension":
        idx_sorted = sorted(range(len(objects)), key=lambda k: biggest_dimension(objects[k]))

    else:
        raise Exception("Please provide a valid method to compute the baseline solution")

    objects = np.array(objects[idx_sorted], dtype=int)
    obj_quants = np.array(obj_quants[idx_sorted], dtype=int)
    quantiles_idxs = compute_quantiles_idxs(obj_quants, num_boxes)
    quantiles_idxs = np.array([0] + quantiles_idxs.tolist())
    BASELINE_SIZES = compute_box_sizes(objects, quantiles_idxs)
    BASELINE_OBJ2BOX = compute_obj2box(objects, BASELINE_SIZES)

    for obj_id in range(len(objects)):
        box_id = BASELINE_OBJ2BOX[obj_id]
        if not fits(objects[obj_id], BASELINE_SIZES[box_id]):
            print(objects[obj_id], BASELINE_SIZES[box_id])

    run_validations(quantiles_idxs, BASELINE_SIZES, BASELINE_OBJ2BOX, objects)
...
def read_csv(file):
    ...
    id_list = prods["Num de referencia"].values.tolist()

    return id_list, objects, quantities


def create_id2box_json(id_list, obj2box, out_file="id2box.json"):

    map = {}
    for prod_id, box_id in zip(id_list, obj2box):
        map[str(prod_id)] = int(box_id) + 1

    with open(out_file, 'w') as file:
        json.dump(map, file)


def create_box_json(box_sizes, out_file="box2size.json"):

    map = {}
    for box_id, size in enumerate(box_sizes):
        id_int = int(box_id) + 1
        size_list = [int(x) for x in size]
        map[id_int] = size_list

    with open(out_file, 'w') as file:
        json.dump(map, file)


def json_dict(objects, obj_quants, object_space, obj2box, box_sizes):

        box_space = total_space_box(box_sizes, obj2box, obj_quants)
        num_bosses, cost = compute_impact(objects, obj_quants,
                                          obj2box, box_sizes)
        R = {
            'percentage': float( round(object_space/box_space*100, 2) ),
            'num_airbags': int(num_bosses),
            'cost_airgbags': float( round(cost, 2) )
        }
...
def main():
    ...
    np.random.seed(1234)
    ID_LIST, OBJECTS, OBJ_QUANTS = read_csv("Packages_Optimization_input.csv")
    OBJECT_SPACE = total_space_obj(OBJECTS, OBJ_QUANTS)
    # Mutacions
    PROB_MUT = 0.05   #[0,1]
    WINDOW_MUT = 10
    ...
    # NUM_BOXES RANGE (both included)
    MIN_NUM_BOXES, MAX_NUM_BOXES = 5, 20
    NUM_ITER = 150

    #Guarda tota la informació i tot el procés
    RESULTS = {
        'objects': {
            'num_objects': int(OBJ_QUANTS.sum()),
            'obj_space': int(total_space_obj(OBJECTS, OBJ_QUANTS))
        },
        'ours': dict(),
        'biggest_dimension': dict(),
        'volume': dict(),
        'diagonal': dict()
    }

    for K in range(MIN_NUM_BOXES, MAX_NUM_BOXES+1):

        ti = time.time()
        BEST_SIZES, BEST_OBJ2BOX = ComputeSizes(OBJECTS, OBJ_QUANTS, K,  # k = num_boxes
                                                NUM_ITER, PROB_MUT, WINDOW_MUT,
                                                ITERS_MUT)

        RESULTS['ours'][K] = json_dict(OBJECTS, OBJ_QUANTS, OBJECT_SPACE,
                                       BEST_OBJ2BOX, BEST_SIZES)

        tf = time.time()
        #print(f"RESULTS[{K}] = ", RESULTS[K])
        print(f"K = {K}.  time: {round(tf-ti, 2)} seconds;  {NUM_ITER} iterations.")

        for baseline in ["biggest_dimension", "volume", "diagonal"]:

            base_sizes, base_obj2box = ComputeBaselineSizes(OBJECTS,
                                                            OBJ_QUANTS,
                                                            K,
                                                            baseline)

            RESULTS[baseline][K] = json_dict(OBJECTS, OBJ_QUANTS, OBJECT_SPACE,
                                             base_obj2box, base_sizes)
            print("baseline", baseline)

        print()

    f_name = f"results_dir/({MIN_NUM_BOXES},{MAX_NUM_BOXES})_{NUM_ITER}.json"
    ...
    if WRITE_JSONS and False:
        create_id2box_json(ID_LIST, BEST_OBJ2BOX)
        create_box_json(BEST_SIZES)
        print()
        print("Els JSON han sobre-escrit la solució anterior.")
...
if __name__ == "__main__":
    main()
    print("ok")
    # TODO les mutacions haurien de modificar el obj2box. Revisar a veure que passa.
