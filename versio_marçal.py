...
def sort_and_compute_quantiles_idxs(objects, obj_quants, num_boxes, method):
    if method == "volume":
        idx_sorted = sorted(range(len(objects)), key=lambda k: volume(objects[k]))
    elif method == "biggest_dim":
...
def ComputeBaselineSizes(_objects, _obj_quants, num_boxes, method="volume"):
    ...
    if method == "volume":
        quantiles_idxs = sort_and_compute_quantiles_idxs(objects, obj_quants, num_boxes, "volume")  # Es reordenen els objectes -> S'han de modificar les variables globals
    elif method == "biggest_dim":
        quantiles_idxs = sort_and_compute_quantiles_idxs(objects, obj_quants, num_boxes, "biggest_dim")  # Es reordenen els objectes -> S'han de modificar les variables globals
...
def main():
    ...
    print("Baseline solution computed by means of quantiles computed from the biggest dimension of each object")
    BIG_DIM_BASELINE_SIZES, BIG_DIM_BASELINE_OBJ2BOX = ComputeBaselineSizes(OBJECTS, OBJ_QUANTS, NUM_BOXES, "biggest_dim")
    display_results(OBJECTS, OBJ_QUANTS, BIG_DIM_BASELINE_OBJ2BOX, BIG_DIM_BASELINE_SIZES)
    plot_results(OBJECTS, BIG_DIM_BASELINE_SIZES, BIG_DIM_BASELINE_OBJ2BOX)
    ...
    #summary(OBJECTS, BIG_DIM_BASELINE_SIZES, BIG_DIM_BASELINE_OBJ2BOX, VOL_BASELINE_SIZES, VOL_BASELINE_OBJ2BOX, DIAG_BASELINE_SIZES, DIAG_BASELINE_OBJ2BOX, BEST_SIZES, BEST_OBJ2BOX)
